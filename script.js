const SCRIPT_VERSION = "1.8"; // Oder deine gewünschte Version
...
let state = {
    ...
    rotationState: {
        ...
        completedSubstituteVipsThisRound: [] // <-- NEU
    },
...
const versionEl = document.getElementById('script-version'); // Element for version display
...
// --- Core Logic ---
function calculateDailyAssignments(targetDateStr, currentR4R5Index, currentMemberIndex, currentSkippedVips, selectedMvpsMap, completedSubstitutes = []) { // Added completedSubstitutes parameter
    if (!targetDateStr || typeof currentR4R5Index !== 'number' || typeof currentMemberIndex !== 'number' || !Array.isArray(currentSkippedVips) || typeof selectedMvpsMap !== 'object' || !Array.isArray(completedSubstitutes)) { console.error("Invalid params"); return { date: new Date(), conductor: { id: 'ERR', name: 'Error' }, vip: { id: 'ERR', name: 'Error'} }; }
    ...
    // Determine VIP (Member) - Now considers completedSubstitutes
    ...
    if (validSkippedVips.length > 0) {
        ...
        while (!potentialVip && loopCheck < memberMembers.length) {
            ...
            if (candidate && !completedSubstitutes.includes(candidate.id)) {
                ...
                console.log(`CalcAssign: Using memberIndex ${vipIndex}: ${potentialVip?.name} (Not a substitute this round)`);
            } else {
                console.log(`CalcAssign: Skipping memberIndex ${vipIndex} (${candidate?.name}) - was substitute or invalid.`);
                tempMemberIndex++;
                loopCheck++;
            }
        }
        ...
        if (potentialVip) {
            ...
            console.warn("CalcAssign: All remaining members were substitutes this round or list is empty after filtering.");
            vip = { id: 'NO_VALID_MEMBER', name: 'No valid Member left', rank: 'Sys' };
        }

        // --- Round End Check ---
        // We check if the *original* index wrapped around during the loop
        // This check is now primarily handled in advanceRotation/handleConfirm for clearing the list.
        if (tempMemberIndex > currentMemberIndex && (currentMemberIndex % memberMembers.length) > (tempMemberIndex % memberMembers.length)) {
             console.log("CalcAssign: Potential round end detected during substitute check.");
        }

    } else {
        vip = { id: 'NO_MEMBERS', name: 'No Members Available', rank: 'Sys' };
    }
...
function advanceRotation(vipAccepted, selectedMvpId = null) { // Handles ACCEPT path ONLY
     ...
     const currentSubstituteList = [...(state.rotationState.completedSubstituteVipsThisRound || [])]; // Get substitute list

     const { vip: proposedVip } = calculateDailyAssignments(currentDateStr, currentR4R5Index, currentMemberIndex, currentSkippedVips, currentSelectedMvps, currentSubstituteList); // Pass substitute list
     ...
     let finalSubstituteList = currentSubstituteList; // Assume list stays the same
     if (memberMembers.length > 0 && (nextMemberIndex % memberMembers.length === 0) && nextMemberIndex > currentMemberIndex) {
         console.log("ADVANCE ROTATION: Round End detected! Clearing Substitute List.");
         finalSubstituteList = []; // Clear the list for the new round
     }
     // --- End Round End Check ---
     ...
     state.rotationState.completedSubstituteVipsThisRound = finalSubstituteList; // Save the (potentially cleared) list
...
function updateFirestoreState() { // Updated to save new substitute list
    const stateToSave={
        ...
        rotationState:{
            ...
            completedSubstituteVipsThisRound: state.rotationState.completedSubstituteVipsThisRound || [] // <-- SAVE
        }
    };
...
function renderSchedule() { // Corrected Simulation Step Logic & Debug Logs & Pass Substitutes
     ...
     const substitutes = state.rotationState.completedSubstituteVipsThisRound || []; // Get current substitutes
     const len = Math.max(7, members.length + skipped.length); // Ensure reasonable length
     let date = new Date(state.rotationState.currentDate + 'T00:00:00Z'); let r4r5Idx = state.rotationState.r4r5Index ?? 0; let memIdx = state.rotationState.memberIndex ?? 0; let tempSkipped = [...skipped]; let tempMvps = JSON.parse(JSON.stringify(state.rotationState.selectedMvps || {}));
     // We don't reset the substitute list for the simulation, as it reflects the *current* round state
     ...
     for (let i = 0; i < len; i++) {
          ...
          console.log(`renderSchedule Loop ${i}: Simulating for ${dateStr} (weekday: ${day})`);

          // Pass the *current* substitute list to the calculation for future days
          const { conductor, vip } = calculateDailyAssignments(dateStr, r4r5Idx, memIdx, tempSkipped, tempMvps, substitutes);
          ...
          // --- Simulation Step (REVISED - Corrected Index Logic) ---
          ...
          let advanceMemberIndexForNextLoop = false;

          if (vip.id && !vip.id.startsWith('NO_') && !vip.id.startsWith('ERROR_') && vip.id !== 'NO_VALID_MEMBER') { // Added NO_VALID_MEMBER check
              ...
              if (wasVipFromSkippedList) {
                  ...
                  advanceMemberIndexForNextLoop = true; // Index should advance even if skipped
                  console.log(`>>> Sim ${i} VIP Check: --> Flagging Member index to advance for next loop because skipped VIP was processed.`);
              } else {
                  advanceMemberIndexForNextLoop = true; // Advance for regular VIP
              }
          } else { console.log(`>>> Sim ${i} VIP Check: --> No valid VIP, not advancing indices.`); advanceMemberIndexForNextLoop = false; }

          if (advanceMemberIndexForNextLoop) { console.log(`>>> Sim ${i} VIP Check: --> Advancing Member index from ${memIdx} for next loop.`); memIdx = (memIdx + 1); }
          else { console.log(`>>> Sim ${i} VIP Check: --> NOT advancing Member index for next loop.`); }
...
function render() { console.log("Rendering UI..."); const theme = localStorage.getItem('theme'); if (theme === 'dark') { document.body.classList.add('dark-mode'); themeToggleBtn.textContent = "Toggle Light Mode"; } else { document.body.classList.remove('dark-mode'); themeToggleBtn.textContent = "Toggle Dark Mode"; } renderMemberList(); renderSkippedVips(); renderCurrentDay(); renderSchedule(); renderStatistics(); renderLastCompletedDay(); undoAdvanceBtn.disabled = !state.previousRotationState; if (versionEl) { versionEl.textContent = SCRIPT_VERSION; } }
...
async function handleConfirmAlternativeVip() { // Corrected to store selected MVP ID and alternative VIP ID & count alt VIP
     ...
     state.rotationState.alternativeVips = currentAlternativeVips;
     state.rotationState.completedSubstituteVipsThisRound = currentSubstituteList; // Save updated substitute list
